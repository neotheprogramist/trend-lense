...
thread_local! {
  static API_KEYS_INDEX: RefCell<ApiIndexes> =  RefCell::new(
    StableBTreeMap::init(
...
mod tests {
    ...
    fn register_multiple_keys() {
        ...
        let second_data = ApiData {
            exchange: Exchange::Coinbase,
            api_key: "coinbase_key".to_string(),
            passphrase: Some("passphrase2".to_string()),
        };

        ApiStore::register_key(&principal, data.clone());
        ApiStore::register_key(&principal, second_data.clone());
        ...
        assert_eq!(
            ApiStore::get_by_api(&principal, &second_data.api_key).unwrap(),
            second_data
        );
    }
}
